...
namespace GPConnect.Provider.AcceptanceTests.Steps
{
    ...
    public class FhirSteps : TechTalk.SpecFlow.Steps
    {
        ...
        public void ISetTheJWTHeaderForGetcarerecordWithConfigPatient(string patient) {
            ...
            And($@"I set the JWT requested record patient NHS number to ""{GlobalContext.PatientNhsNumberMap[patient]}""");
        }
        ...
        {
            Given($@"I author a request for the ""{recordSectionCode}"" care record section for patient with NHS Number ""{GlobalContext.PatientNhsNumberMap[patient]}""");
        }
        ...
        public void ThenIAmRequestingTheRecordForPatientWithNHSNumber(string nhsNumber)
        {
            Given($@"I set the JWT requested scope to ""{JwtConst.Scope.kPatientRead}""");
            And($@"I set the JWT requested record patient NHS number to ""{nhsNumber}""");
            FhirContext.FhirRequestParameters.Add(FhirConst.GetCareRecordParams.kPatientNHSNumber, FhirHelper.GetNHSNumberIdentifier(nhsNumber));
        ...
        public void GivenIAmRequestingTheRecordForConfigPatientUsingAFHirStringParameter(string patient)
        {
            Given($@"I set the JWT requested scope to ""{JwtConst.Scope.kPatientRead}""");
            And($@"I set the JWT requested record patient NHS number to ""{GlobalContext.PatientNhsNumberMap[patient]}""");
            FhirContext.FhirRequestParameters.Add(FhirConst.GetCareRecordParams.kPatientNHSNumber, new FhirString(GlobalContext.PatientNhsNumberMap[patient]));
        }
        ...
        public void GivenIAmRequestingTheRecordForConfigPatientOfSystem(string patient, string system)
        {
            Given($@"I set the JWT requested scope to ""{JwtConst.Scope.kPatientRead}""");
            And($@"I set the JWT requested record patient NHS number to ""{GlobalContext.PatientNhsNumberMap[patient]}""");
            FhirContext.FhirRequestParameters.Add(FhirConst.GetCareRecordParams.kPatientNHSNumber, FhirHelper.GetIdentifier(system, GlobalContext.PatientNhsNumberMap[patient]));
        }
        ...
        public void getPractitionerConnection()
        {
            ...
            //    param.Add("system", "http ://fhir.nhs.net/Id/sds-user-id");
                ...
                FhirContext.FhirResponseResource = fhirClient.Search<Appointment>(param);
            }
            catch (Exception e)
            {
                Log.WriteLine("HERE3");
